...
/*Function retrieves json object from the players currently live streaming and displays on playerList.*/
    function getStreamList(streamPlayerList) {
        ...
        $.getJSON("https://api.twitch.tv/kraken/channels/" + streamPlayerList, function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                console.log("name " + data[i].channel.name);
               // $("#streamlist").append('<li id="' + data[i].channel.name + '" class="streams"' + '"></li>');
                //$('#' + data[i].channel.name).append('<img class="small_img" src="' + data[i].channel.logo + '">');
               // $('#' + data[i].channel.name).append('<img class="cap_img" src="' + data[i].channel.screen_cap_url_medium + '">');
                //$('#' + data[i].channel.name).append('<div class="topstrip" id="' + data[i].channel.name + '"> ' + '<span class="streamviews">' + data[i].channel_count + '</span>' + ' </div>');
               // $('#' + data[i].channel.name).append('<div class="bottomstrip" id="' + data[i].channel.name + '"> ' + '<span class="streamname">' + data[i].channel.name + '</span>' + ' </div>');
                


                
                //Allows appended elements to be draggable
//                $('#' + data[i].channel.name).draggable({
